...
| | |
|---|---|
| **v1 board** (CF slot, `board/`) | **Working.** Boots, plays, restores, syncs. Reads 19 MB/s, writes up to 14 MB/s sustained over FireWire. Running a 256 GB SD (via FC1307A CF adapter) today. |
| **3rd-gen iPod** | **Working** (same board, same bitstream). Boots, plays, restores, syncs; 16.9 MB/s reads through the bridge. The 3G's disk-mode handoff sets PIO4 and its boot loader is a UDMA host; both needed gateware care (see below). |
...
Tested on: iPod 1st gen (firmware 1.5) and 3rd gen, Transcend industrial
2 GB CF, retail Lexar Professional 800x, a 16 GB CF, FC1307A SD-CF adapter
+ 256 GB SD, Mac OS X 10.6 + iTunes over FireWire. 2G iPods use the same
TSB43AA82 and *should* work; reports welcome.
...
data setup/hold on every strobe edge, spec-ordered burst termination, every
SET FEATURES transfer-mode request rewritten to UDMA2 at the card while
IDENTIFY reports back whatever mode the host asked for (the 3G's disk-mode
handoff sets PIO4, verifies it took, then does READ DMA anyway), a features
context that expires at each command (a lingering SET FEATURES turned a later
bare SET MULTIPLE count into garbage), and power-cut recovery. That last one
is the infamous flash-mod "skipping songs" symptom, finally explained: the
iPod cuts DRIVE POWER a few seconds after issuing STANDBY. The card loses
its SET MULTIPLE state; the OS assumes it survived (the original Toshiba's
power-on default was agreeable, many cards' is not) and every READ MULTIPLE
aborts forever after. The bridge re-initializes the card after every power
cut and restores SET MULTIPLE itself. The
...
## Capacity limit: LBA28

Both of the iPod's masters are 2001-era LBA28 hosts, and the iSphynxII
truncates addresses to 28 bits and **wraps back to sector 0** (verified by
raw reads: sector 268,435,456 returns the partition map). Cards larger than
137.4 GB (128 GiB) enumerate at full size, but everything past the wall
aliases onto the start of the disk; writes there destroy the partition map.
Partition the card so the HFS+ volume ends below 137 GB. The FPGA cannot
extend this: the upper address bits never reach the ATA cable.
