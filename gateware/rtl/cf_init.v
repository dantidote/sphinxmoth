...
module cf_init #(
    ...
    localparam I_WAIT=0, I_PGO=1, I_PWAIT=2, I_PEVAL=3,
               ...
               I_M1=26, I_M1W=27, I_M2=28, I_M2W=29;
    reg [4:0]  is_;
    reg [23:0] tick;
    reg [14:0] budget;
    reg [2:0]  dump_idx;   // fail autopsy: regs 1..6 (err, SC, LBA0/1/2, dev)
    reg        mdone;      // SET MULTIPLE step completed (shares the BSY poll)
...
`else
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            I_P2EVAL: begin
                // BSY clear: first pass -> restore SET MULTIPLE (the iPod CUTS
                // DRIVE POWER ~8s after STANDBY; on re-init the OS re-issues
                // SET FEATURES but assumes multiple-mode survived -- the
                // Toshiba's power-on default did, many cards' does not, and
                // every observed master negotiates count 1); second pass ->
                // restore signature
                if (!last_status[7]) is_ <= mdone ? I_SG2 : I_M1;
                else if (budget==0) begin
                    // post-EF timeout: full autopsy. post-C6 timeout: log it but
                    // still restore the signature -- SET MULTIPLE is optional,
                    // leaving the bus half-configured over it is not
                    fail_ev <= 1;
                    if (mdone) is_ <= I_SG2;
                    else begin dump_idx<=3'd1; is_<=I_DGO; end
                end
                else begin budget <= budget-1'b1; tick <= T_2MS; is_ <= I_P2GO; end
            end
            I_M1:  begin xop<=1; xaddr<=3'd2; xwdata<=16'h0001; xgo<=1; is_<=I_M1W; end
            I_M1W: if (!xgo && !xbusy) is_ <= I_M2;
            I_M2:  begin xop<=1; xaddr<=3'd7; xwdata<=16'h00C6; xgo<=1; is_<=I_M2W; end  // SET MULTIPLE 1
            I_M2W: if (!xgo && !xbusy) begin
                       mdone <= 1'b1; budget <= BUDGET2; tick <= T_2MS; is_ <= I_P2GO;
                   end
