...
module udma_device #(
    ...
    input             cmd_start       // pulse: new ATA command (clears staged carry-over)
);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (cmd_start) ld_r <= 1'b0;      // staged word from an aborted
                                              // command must not leak into this one
            case (st)
            S_IDLE: begin
            ...
            S_ARB: begin                          // wait for the DMACK grant
                if (~hs_dmack_n) begin
                    tmr <= T_ENV;
                    st  <= S_ENV;
                end
