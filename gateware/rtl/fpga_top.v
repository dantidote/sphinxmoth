...
`endif
    ...
    // host reset: async assert, SYNCHRONIZED release (raw deassertion into
    // ~100 async-reset flops risks recovery/removal on random subsets)
    reg [1:0] hrst_sync = 2'b00;
    always @(posedge clk or negedge HS_RESET_N)
        if (!HS_RESET_N) hrst_sync <= 2'b00;
        else             hrst_sync <= {hrst_sync[0], 1'b1};
    ...
    wire cf_xfer = ~CF_DMACK_N
                 | (~CF_CS0_N & ({CF_A2,CF_A1,CF_A0} == 3'b000)
                    & (~CF_IOR_N | ~CF_IOW_N));
    reg cfx_q = 1'b0;                    // 1-FF sync: async pins feed a
    always @(posedge clk) cfx_q <= cf_xfer;  // multi-bit counter load otherwise
    reg [21:0] act_hold = 22'd0;
    always @(posedge clk)
        act_hold <= cfx_q ? {22{1'b1}}
    ...
    // Define DEBUGLESS to omit the logger (and let synthesis sweep the debug
    // counters): frees ~290 logic + ~96 RAM slices (28% of the device!), but
    // the next field mystery starts blind. The default build keeps it.
`ifdef DEBUGLESS
    assign UART_TX = 1'b1;
`else
