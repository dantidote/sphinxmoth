...
//
// SEQUENCING (this build = CONCURRENT reads, STORE-AND-FORWARD writes):
//   READ  : host engine fills the FIFO from the CF while the device engine
//           drains it to the iSphynx (producer/consumer, no barrier).
//   WRITE : device engine captures the whole iSphynx delivery first, THEN the
//           host engine opens the CF burst (hardware: concurrent write bursts
//           stall with the card never asserting DDMARDY#). fifo_afull is the
//           >FIFO-sized-command safety valve; DEPTH=4096 sizes that valve.
//
...
module interposer_top #(
    ...
    output [7:0]  DBG_HOSTQ,    // {host_st_last, cf_dmarq_seen, cf_rdy_seen, 00}
    ...
    reg [7:0]  sel88_bits;         // one-hot UDMA "selected" byte, decoded ONCE
                                   // at SET FEATURES time (MWDMA: no observed
                                   // host requests it; selected always clears)
    always @(posedge CLK or negedge RST_N) begin
        if (!RST_N) begin
            ...
            if (iow_s & ~iow_q2 & ~cs0_s & cs1_s) begin        // IOW# rising, cmd block
                ...
                // a command CONSUMES the features context: without this, the
                // OS's F03 (PIO fallback) lingers and the SC of a later bare
                // SET MULTIPLE gets mode-rewritten to 0x42 = count 66 -> ABRT
                // -> every READ/WRITE MULTIPLE fails -> playback skip storm
                if (adr_s == 3'd7) feat_last <= 8'h00;
                if ((adr_s == 3'd2) & (feat_last == 8'h03))
                    sel88_bits <= (hs_dd_s[7:4] == 4'h4) ? (8'h01 << hs_dd_s[2:0]) : 8'h00;
            end
        end
    end
    ...
    // words 63/88 readback curation: "supported" (low byte) stays truthful,
    // "selected" (high byte) mirrors the host's LAST requested mode -- the 3G
    // disk-mode qualifier sets PIO4 and verifies it took via IDENTIFY, while
    // the card (pinned at UDMA2 by the SC rewrite) would truthfully contradict
    wire [15:0] w63_map_live = {8'h00, cf_dd_s[7:0]};
    wire [15:0] w88_map_live = {sel88_bits, cf_dd_s[7:0]};
    wire [15:0] w63_new_lat  = pp_sess ? 16'h0 : {8'h00, id_orig[7:0]};
    wire [15:0] w88_new_lat  = pp_sess ? 16'h0 : {sel88_bits, id_orig[7:0]};
    always @(posedge CLK or negedge RST_N) begin
        if (!RST_N) begin
            id_active <= 1'b0; id_wcnt <= 8'd0; id_csum_delta <= 8'd0;
            id_orig   <= 16'h0;
            pp_sess   <= 1'b0;
        end else begin
            ...
            if (cmd_valid) begin
                ...
                if ((id_wcnt == 8'd0) & id_orig_bad)
                    id_csum_delta <= id_csum_delta
                                   + id_orig[7:0] + id_orig[15:8] - 8'h5A - 8'h04;
                if (id_wcnt == 8'd63)
                    id_csum_delta <= id_csum_delta
                                   + id_orig[7:0] + id_orig[15:8]
                                   - w63_new_lat[7:0] - w63_new_lat[15:8];
                if (id_wcnt == 8'd88)
                    id_csum_delta <= id_csum_delta
                                   + id_orig[7:0] + id_orig[15:8]
                                   - w88_new_lat[7:0] - w88_new_lat[15:8];
                if (id_wcnt == 8'd255) id_active <= 1'b0;
                id_wcnt <= id_wcnt + 8'd1;
            end
        end
    end
    ...
    // ANY transfer-mode request (PIO 0x0x, MWDMA 0x2x, UDMA 0x4x) becomes
    // UDMA2 at the card: the card-side engine speaks only UDMA2, and the 3G
    // disk-mode handoff really does set PIO4 then issue READ DMA anyway
    ...
    wire        id0_hit   = id_rd & (id_wcnt == 8'd0) & id_w0_bad;   // fixed disk
    wire        id63_hit  = id_rd & (id_wcnt == 8'd63);   // selected-mode curation
    wire        id88_hit  = id_rd & (id_wcnt == 8'd88);   // (pp: no DMA at all)
    wire        id255_hit = id_rd & (id_wcnt == 8'd255) & (cf_dd_s[7:0] == 8'hA5);
    wire [15:0] id_val    = id0_hit ? 16'h045A
                          : id63_hit ? (pp_sess ? 16'h0000 : w63_map_live)
                          : id88_hit ? (pp_sess ? 16'h0000 : w88_map_live)
    ...
    // ------------------------------------------------------------------------
    localparam SEQ_IDLE = 1'b0, SEQ_RUN = 1'b1;
    reg        seq;
    reg        dir_w_l;
    reg        want_host, want_dev;                     // request, HELD until ack
    wire       host_busy, dev_busy;
    ...
    always @(posedge CLK or negedge RST_N)
        if (!RST_N) begin prod_complete<=1'b0; fifo_room<=1'b1; cons_ready<=1'b0;
            ...
            // concurrent.
            cons_ready    <= dir_w_l ? ((cap_done & (~fifo_empty | wr_pend))
                                        | ge_thresh)
    ...
    always @(posedge CLK or negedge RST_N)
        if (!RST_N) begin cf_dmarq_seen <= 1'b0; host_st_last <= 4'hF; cf_rdy_seen <= 1'b0; end
        else begin
            if (dbg_start_r) begin cf_dmarq_seen <= 1'b0; host_st_last <= 4'hF; cf_rdy_seen <= 1'b0; end
            else begin
    ...
    assign DBG_WCAP   = words_cap[16] ? 16'hFFFF : words_cap[15:0];  // 256-sector cmd saturates, not aliases to 0
    assign DBG_DMACKF = hs_dmackf;
    assign DBG_HOSTQ  = {host_st_last, cf_dmarq_seen, cf_rdy_seen, 2'b0};

    // ------------------------------------------------------------------------
    // burst watchdog: ~4s with no FIFO progress kills the burst. On fire,
    ...
    udma_device #(.CLK_MHZ(CLK_MHZ)) u_dev (
        .cmd_start  (dbg_start_r),
        .clk(CLK), .rst_n(eng_rst_n),
        .go(dev_go), .dir_write(dir_w_l), .busy(dev_busy), .done(),
    ...
    udma_host #(.CLK_MHZ(CLK_MHZ)) u_host (
        ...
        .dir_write(dir_w_l), .busy(host_busy), .done(),
    ...
    wire [7:0] live_stat = {1'b0, seq, dir_w_l, host_busy, dev_busy, fifo_empty, fifo_full, 1'b0};
    reg  [7:0] stat_wd;
    always @(posedge CLK or negedge RST_N)
