...
module udma_host #(
    ...
    always @(posedge clk or negedge rst_n) begin
        ...
        else if (cap_valid & ((st == S_RD_DATA) | (st == S_RD_DRAIN)))
            dbg_wcnt <= dbg_wcnt + 16'd1;         // words actually stored, not
                                                  // pre-grant/termination glints
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            tmr <= 0; cur <= 0; ld <= 0; dmarq_fresh <= 1'b1; wsent <= 17'd0;
        end else begin
            ...
            if (cmd_start) begin
                ...
                ld <= 1'b0;                        // a word staged by an ABORTED
                                                   // command must not leak in as
                                                   // word 0 of this one
            end
