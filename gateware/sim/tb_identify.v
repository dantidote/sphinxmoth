...
module tb_identify;
    ...
    reg [7:0] cf_last_sc  = 0;
    always @(posedge CF_IOW_N) begin
        if (!CF_CS0_N && {CF_A2,CF_A1,CF_A0} == 3'd2)
            cf_last_sc = CF_DD[7:0];
    ...
    integer errs = 0;
    integer bsum;
    task read_identify(input [15:0] exp63, input [15:0] exp88, input [8*16-1:0] label);
        integer k;
        begin
            ...
            if (rx[63] !== exp63 || rx[88] !== exp88) begin
                errs = errs + 1;
                $display("FAIL %0s: w63=%04x w88=%04x (want %04x/%04x)",
                         label, rx[63], rx[88], exp63, exp88);
            end
    ...
    initial begin
        ...
        // session 1: iSphynx, no SET FEATURES (belief = default UDMA2):
        // w63 selected cleared (UDMA active), w88 shows UDMA2 selected
        read_identify(16'h0007, 16'h0407, "isphynx ");

        // session 1b: host requests PIO4 (the 3G disk-mode qualifier move) --
        // both selected fields clear, supported truthful, checksum valid
        hs_write(3'd1, 16'h0003); hs_write(3'd2, 16'h000C); hs_write(3'd7, 16'h00EF);
        read_identify(16'h0007, 16'h0007, "pio4-req");

        // session 1c: host requests UDMA1 -- w88 selected follows the request
        hs_write(3'd1, 16'h0003); hs_write(3'd2, 16'h0041); hs_write(3'd7, 16'h00EF);
        read_identify(16'h0007, 16'h0207, "udma1req");

        // session 2: PP5002 (C90 first) -- word0 fixed, DMA words zeroed
        hs_write(3'd7, 16'h0090);
        read_identify(16'h0000, 16'h0000, "pp5002  ");

        // power commands pass through untouched: the iPod cuts drive power
        // after STANDBY regardless, so the card should get its orderly park
        // (the old E0->E5 shim treated a disproven mechanism)
        hs_write(3'd7, 16'h00E0); if (cf_last_cmd !== 8'hE0) begin errs=errs+1; $display("FAIL: E0 -> %02x (want passthrough)", cf_last_cmd); end
        hs_write(3'd7, 16'h00E1); if (cf_last_cmd !== 8'hE1) begin errs=errs+1; $display("FAIL: E1 -> %02x (want passthrough)", cf_last_cmd); end
        $display("power cmds: pass through untouched");

        // features context must EXPIRE at each command: SET FEATURES (F03)
        // then a bare SET MULTIPLE -- its count must reach the card intact
        hs_write(3'd1, 16'h0003); hs_write(3'd2, 16'h000C); hs_write(3'd7, 16'h00EF);
        if (cf_last_sc !== 8'h42) begin errs=errs+1; $display("FAIL: F03 SC -> %02x (want 42 rewrite)", cf_last_sc); end
        hs_write(3'd2, 16'h0010); hs_write(3'd7, 16'h00C6);
        if (cf_last_sc !== 8'h10) begin errs=errs+1; $display("FAIL: bare SET MULTIPLE count -> %02x (want 10)", cf_last_sc); end
        if (cf_last_cmd !== 8'hC6) begin errs=errs+1; $display("FAIL: C6 -> %02x", cf_last_cmd); end
        $display("feat context: F03 SC rewritten, post-command SC untouched");

        if (errs == 0) $display("PASS: IDENTIFY patcher + mode curation + feature-context expiry");
